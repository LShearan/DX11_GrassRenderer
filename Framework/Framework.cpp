...
public:
	...
	{
		const float clearColor[] = { 0.3f, 0.5f, .7f, 1.f };
...
private:
	...
	void initD3D()
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			IDXGIDevice* device;
			m_pD3DDevice->QueryInterface(__uuidof(IDXGIDevice), (void**)&device);
			...
			factory->CreateSwapChain(m_pD3DDevice.Get(), &sd, &m_pSwapChain);
	...
	void CreateDepthBuffer(const UINT width, const UINT height)
	{
		...
		hr = m_pD3DDevice->CreateDepthStencilView(m_pDepthStencil.Get(), 0, m_pDepthStencilView.GetAddressOf());
		if (FAILED(hr))
		{
